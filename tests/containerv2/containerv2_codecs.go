...
func (stream *ContainerStream) Stream() iter.Seq2[BingenFieldInfo, *BingenValue] {
	return func(yield func(BingenFieldInfo, *BingenValue) bool) {
		...
		if buff.ReadUInt8() == uint8(0) {
			...
			e := buff.ReadInt() // slice len
			if e < 0 {
				stream.err = fmt.Errorf("bingen: invalid slice length %d", e)
				return
			...
			if e > MaxContainerLength {
				stream.err = fmt.Errorf("bingen: slice length %d exceeds MaxContainerLength %d", e, MaxContainerLength)
				return

			}
			if rem := buff.Remaining(); rem >= 0 && e > rem {
				stream.err = fmt.Errorf("bingen: slice length %d exceeds remaining bytes %d", e, rem)
				return
