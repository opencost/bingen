...
const (
	...
	GeneratorPackageName string = "aliasnil"
...
// MarshalBinary serializes the internal properties of this Holder instance
// into a byte array
func (target *Holder) MarshalBinary() (data []byte, err error) {
...
// MarshalBinary serializes the internal properties of this Holder instance
// into an io.Writer.
func (target *Holder) MarshalBinaryTo(writer io.Writer) error {
...
// MarshalBinaryWithContext serializes the internal properties of this Holder instance
// into a byte array leveraging a predefined context.
func (target *Holder) MarshalBinaryWithContext(ctx *EncodingContext) (err error) {
	...
	if []string(target.Tags) == nil {
		buff.WriteUInt8(uint8(0))
	} else {
		buff.WriteUInt8(uint8(1))
		buff.WriteInt(len([]string(target.Tags)))
		for v1 := range []string(target.Tags) {
			if ctx.IsStringTable() {
				v2 := ctx.Table.AddOrGet([]string(target.Tags)[v1])
				buff.WriteInt(v2)
			} else {
				buff.WriteString([]string(target.Tags)[v1])
			}
		}
	}
...
// the Holder type
func (target *Holder) UnmarshalBinary(data []byte) error {
...
// the Holder type
func (target *Holder) UnmarshalBinaryFromReader(reader io.Reader) error {
...
// the Holder type
func (target *Holder) UnmarshalBinaryWithContext(ctx *DecodingContext) (err error) {
	// panics are recovered and propagated as errors
	defer func() {
		if r := recover(); r != nil {
			if e, ok := r.(error); ok {
	...
	if version > DefaultCodecVersion {
		return fmt.Errorf("Invalid Version Unmarshalling Holder. Expected %d or less, got %d", DefaultCodecVersion, version)
	}
