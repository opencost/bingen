...
func jsonEncodeString(_ *bytes.Buffer, _ string, _ string, _ string) {

}

func jsonEncodeFloat64(_ *bytes.Buffer, _ string, _ float64, _ string) {

}
func jsonEncode(_ *bytes.Buffer, _ string, _ interface{}, _ string) {
...
func preProcessAssetSet(assetSet *AssetSet) {
	...
	assetSet.assets = make(map[string]Asset, length)
	for key, asset := range assetSet.Any {
		assetSet.assets[key] = asset
	}
	for key, asset := range assetSet.Cloud {
		assetSet.assets[key] = asset
	}
	for key, asset := range assetSet.ClusterManagement {
		assetSet.assets[key] = asset
	}
	for key, asset := range assetSet.Disks {
		assetSet.assets[key] = asset
	}
	for key, asset := range assetSet.Network {
		assetSet.assets[key] = asset
	}
	for key, asset := range assetSet.Nodes {
		assetSet.assets[key] = asset
	}
	for key, asset := range assetSet.LoadBalancers {
		assetSet.assets[key] = asset
	}
	for key, asset := range assetSet.SharedAssets {
		assetSet.assets[key] = asset
	}
}
