...
const BingenConfigurationCode string = `
var (
...
const BingenFieldStreamSupportCode string = `
...
const BingenNewStreamForCode string = `
...
const StringTableReaderCode string = `
...
// StringTableWriterCode is the source code for the string table encoding implementation.
const StringTableWriterCode string = `
...
const BinaryCodecInterfaceCode string = `
...
const BinaryCodecContextsCode string = `
...
const BingenStreamFormatCode = `
// %sStream is a single use field stream for the contents of an %s instance. Instead of creating an instance and populating
...
// Closes closes the internal io.Reader used to read and parse the %s fields.
// This should be called once the stream is no longer needed.
func (stream *%sStream) Close() {
...
// Error returns an error if one occurred during the process of streaming the %s
// This can be checked after iterating through the Stream().
func (stream *%sStream) Error() error {
	return stream.err
}

// New%sStream creates a new %sStream, which uses the io.Reader data to stream all internal fields of an %s instance
func New%sStream(reader io.Reader) BingenStream {
